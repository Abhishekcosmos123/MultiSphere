...
export function SkillInput({ skills, setSkills }: { skills: string[], setSkills: (val: string[]) => void }) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if ((e.key === "Enter" || e.key === ",") && input.trim()) {
      e.preventDefault()
      if (!safeSkills.includes(input.trim())) {
        setSkills([...safeSkills, input.trim()])
        setInput("")
      }
    } else if (e.key === "Backspace" && input === "" && safeSkills.length) {
      setSkills(safeSkills.slice(0, -1))
    }
  }

  const removeSkill = (index: number) => {
    setSkills(safeSkills.filter((_, i) => i !== index))
  }
